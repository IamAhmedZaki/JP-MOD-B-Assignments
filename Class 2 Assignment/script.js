...
const createBrandOption=(mobiles)=>{
let selectOne=document.querySelector('#brand')
Object.keys(mobiles).forEach((keys)=>{
    let options=document.createElement('option')
    options.value=keys
    options.textContent=keys
    selectOne.append(options)
    
})

}

createBrandOption(mobiles);

const createModelOptions=(mobiles)=>{
    let selectOne=document.querySelector('#brand').value
    let selectTwo=document.querySelector('#model')

    selectTwo.innerHTML = '<option>Select Model</option>';

    Object.keys(mobiles[selectOne]).forEach((keys)=>{
        let options=document.createElement('option')
        options.value=keys
        options.textContent=keys
        selectTwo.append(options)
    })
}

// createModelOptions(mobiles)
